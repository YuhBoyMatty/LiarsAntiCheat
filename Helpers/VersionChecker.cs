    ...
    public class VersionChecker
    {
        ...
        private static async Task<bool> GetVersionFromUrlAsync(string url)
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    LiarsAntiCheatMod.mls.LogError(
                        $"Failed to get the latest version from {url} for {LiarsAntiCheatMod.modName} Mod. Error: {e.Message}"
                    );
                    return false;
                }
            }
        ...
        private static string BuildUserAgentString()
        {
            ...
            return $"{LiarsAntiCheatMod.modGUID}/{LiarsAntiCheatMod.modVersion} ({osInfo}; {architecture})";
        }
